...
use cosmwasm_std::{ensure_eq, Addr, Coin, CosmosMsg, Deps, DepsMut, Env, MessageInfo, Reply, StdError, SubMsg};
...
fn perform_action(deps: DepsMut, info: MessageInfo, env: Env, adapter: BoardAdapter) -> BoardResult {
    let account_registry = adapter.account_registry(deps.as_ref())?;
    ...
    let msgs: Vec<SubMsg> = match tile_action {
        ...
        TileAction::Action { action } => {
            if let Some(action) = action {
                ...
                match action_type {
                    ActionType::Lend => create_lending_message(deps.as_ref(), env, &adapter, &info.sender, required_funds, info.funds)?,
                }
            } else {
...
fn create_lending_message(deps: Deps, env:Env, adapter: &BoardAdapter, user: &Addr, required_funds: Vec<Coin>, added_funds: Vec<Coin>) -> Result<Vec<SubMsg>, BoardError> {
