use crate::{
    contract::BoardResult, state::{CONFIG, ONGOING_ACTIONS, STATUS, TILES}
};

use abstract_adapter::objects::AccountId;
use common::board::{BoardAdapter, BoardQueryMsg, ConfigResponse, OngoingActionResponse, StatusResponse, TileActionResponse};
...
pub fn query_handler(
    ...
    match msg {
        ...
        OngoingActionFromCanonical { addr } => to_json_binary( &query_ongoing_action_from_canonical(deps, addr)?),
        TileAction { tile_id } => to_json_binary(&query_tile_action(deps, tile_id)?),
    }
    .map_err(Into::into)
}

fn query_ongoing_action_from_canonical(deps: Deps<cw_orch::prelude::Empty>, addr: cosmwasm_std::CanonicalAddr) -> StdResult<OngoingActionResponse>{
