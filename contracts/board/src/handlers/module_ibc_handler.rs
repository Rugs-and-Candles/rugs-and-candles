...
pub fn module_ibc_handler(
    ...
    match server_msg {
        BoardIbcMsg::RegisterAction { user, tile_number } => {
            ...
            return handle_register_action(deps, env, module, user_addr, tile_id);
        }
    }
}

fn handle_register_action(deps: DepsMut, env:Env, adapter: BoardAdapter, user_addr: Addr, tile_id: TileId) -> BoardResult {
    let tile_action = TILES.load(deps.storage, tile_id)?;

    let sub_msgs = match tile_action {
        TileAction::Candle { n_tile: _ }=> match_tile_action_to_message(tile_action, &deps, &adapter, &user_addr, vec![], tile_id, env)?, 
        TileAction::Rugg{ n_tile: _} => match_tile_action_to_message(tile_action, &deps, &adapter, &user_addr, vec![], tile_id, env)?,
